...
use crate::message::{Message, MessageCode, MessageType};
...
impl IcmpConnection {
    pub fn new(remote: SocketAddr, id: u16) -> anyhow::Result<Self> {
        Ok(Self {
            socket: setup_socket(format!("0.0.0.0:0").parse()?)?,
            remote: remote.into(),
            buffers: Buffers::new(),
            id,
        })
    ...
    pub fn recv(&mut self, seq: u16) -> anyhow::Result<Option<Message>> {
        ...
        while started.elapsed() < timeout() {
            match self.recv_once() {
                Ok(Some(msg)) if msg.ours(id, seq) => return Ok(Some(msg)),
...
fn setup_socket(local: SocketAddr) -> anyhow::Result<Socket> {
