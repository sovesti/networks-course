...
impl Session {
    pub fn new(host: String, addr: SocketAddr, bytes: usize) -> Self {
        Self { host, addr, bytes }
    }
}

impl Display for Session {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        let addr = if self.host == self.addr.ip().to_string() {
            self.host.clone()
        } else {
            format!("{} [{}]", self.host, self.addr.ip())
        };
        write!(f, "Pinging {addr} with {} bytes of data:", self.bytes)
    }
}
...
impl Display for Stats {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        writeln!(f, "")?;
        writeln!(f, "Ping statistics for {}:", self.session.addr.ip())?;
