...
Приложение реализовано на фреймворке Dioxus, для сборки понадобятся Rust и консольное приложение ```dioxus-cli```, инструкцию по установке можно найти в документации фреймворка: https://dioxuslabs.com/learn/0.7/getting_started/. Тестировалось только под Windows. Запуск:
...
Приложение может работать и как сервер, и как клиент. Через ```config.toml``` можно настроить порт, на котором приложение будет работать в качестве сервера.

Чтобы начать работу в качестве клиента, нужно указать адрес сервера и нажать "Подключиться" (по умолчанию указан адрес сервера самого приложения, то есть приложение подключится само к себе). Чтобы стать сервером и ожидать подключения от клиента, нужно выбрать "Создать сервер" и дождаться подключения другого приложения.

Данные передаются через TCP-соединение в виде описаний линий, которые проводит второй участник сессии на холсте. Передача происходит в обе стороны.

Пример работы (запускаются два приложения, у одного в конфиге порт 3060, у второго 3062):

https://github.com/user-attachments/assets/b0a81c58-ed6d-4c9a-bb6f-6708016f9ded
